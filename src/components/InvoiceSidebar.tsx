...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="sidebar-section-label mt-6 mb-3 first:mt-0">{children}</p>
  );
}
...
function validateCnpj(cnpj: string): boolean {
  ...
  let rest = sum % 11;
  if (rest < 2) {
    if (parseInt(digits[12]) !== 0) return false;
  } else {
    if (parseInt(digits[12]) !== 11 - rest) return false;
  }
  sum = 0;
...
export default function InvoiceSidebar({
  ...
  const handleEmailChange = (
    field: "companyEmail" | "clientEmail",
    value: string,
  ) => {
  ...
  const handleDocChange = (
    field: "companyCnpj" | "clientCnpjCpf",
    value: string,
  ) => {
  ...
  return (
    <aside className="no-print w-full md:w-[340px] lg:w-[380px] shrink-0 md:h-screen overflow-y-auto border-b md:border-b-0 md:border-r border-border bg-card flex flex-col">
      <div className="p-5 pt-14 md:pt-5 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="font-mono-display text-lg font-bold tracking-tight">
            Meu <span className="text-accent">Recibo</span>{" "}
            <span style={{ color: "hsl(51 100% 40% / 0.75)" }}>Online</span>
          </h1>
          <button
            onClick={resetData}
        ...
        <Field
          ...
          onChange={(v) => handleDocChange("companyCnpj", v)}
          placeholder="00.000.000/0000-00"
          error={docErrors.company}
        />
        <Field
          label="E-mail"
          value={data.companyEmail}
          onChange={(v) => handleEmailChange("companyEmail", v)}
          placeholder="contato@empresa.com"
          error={emailErrors.company}
        />
        <Field
          label="Endereço"
        ...
        <Field
          label="CNPJ / CPF"
          value={data.clientCnpjCpf}
          onChange={(v) => handleDocChange("clientCnpjCpf", v)}
          placeholder="000.000.000-00"
          error={docErrors.client}
        />
        <Field
          label="E-mail"
          value={data.clientEmail}
          onChange={(v) => handleEmailChange("clientEmail", v)}
          placeholder="cliente@email.com"
          error={emailErrors.client}
        />
        <Field
          label="Endereço"
          value={data.clientAddress}
          onChange={(v) => updateField("clientAddress", v)}
          placeholder="Av. Brasil, 456 - Rio de Janeiro, RJ"
        />
        ...
        <div className="mb-3">
          ...
          <select
            ...
            onChange={(e) =>
              updateField(
                "paymentMethod",
                e.target.value as InvoiceData["paymentMethod"],
              )
            }
        ...
        {data.paymentMethod === "PIX" && (
          <>
            ...
            <Field
              label="Chave PIX"
              value={data.pixKey}
              onChange={(v) => updateField("pixKey", v)}
            ...
            {data.paymentMethod === "Transferência Bancária" && (
              <>
                <Field
                  ...
                  onChange={(v) => updateField("bankAgency", v)}
                  placeholder="0001"
                />
                <Field
                  label="Conta"
        ...
        {data.items.map((item, i) => (
          <div
            key={item.id}
            className="mb-3 p-3 rounded-md border border-border bg-muted/30"
          >
